...
/// <summary>
/// Kiểm tra chuyên sâu tính năng mã hóa mTLS và quản lý chứng chỉ / khóa bí mật:
...
public class MtlsAndKeyStoreTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "MtlsTest_" + Guid.NewGuid().ToString("N"));
    ...
    public void KeyStore_InstallsAndFindsClientCertificateWithPrivateKey()
    {
        ...
        // Cài đặt chứng chỉ mTLS
        _keyStore.InstallCertificate(certPem, key, _config);

        Assert.NotNull(_config.ClientCertThumbprint);
        Assert.NotNull(_config.CertStoreLocation);

        // Tìm lại chứng chỉ trong Store
